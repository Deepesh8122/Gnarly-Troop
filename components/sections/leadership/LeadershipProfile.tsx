...
export default function LeadershipProfile({ person }: Props) {
  const paragraphs =
    ...
    (person.bio ? [person.bio] : ["Biography coming soon."]);
      ...
      <div className={styles.profile}>
        ...
        <header className={styles.profileHeader}>
          <div className={styles.nameBlock}>
            ...
            {person.division && (
              <p className={styles.division}>{person.division}</p>
            )}
          </div>
          {person.linkedin && (
        ...
        {person.education && (
          <section className={styles.education} aria-labelledby="education-heading">
            <h2 id="education-heading" className={styles.sectionLabel}>
              Education
            </h2>
              ...
              {person.articles!.map((art) => (
                <li key={art.href}>
                  <Link href={art.href} className={styles.articleCard}>
                    {art.type && (
                      <span className={styles.articleType}>{art.type}</span>
                    )}
                    <h3>{art.title}</h3>
                    <p>{art.excerpt}</p>
